...
static inline void lui_bytes_init (LvtkBytes* b, size_t size) {
    b->data = NULL;
    b->size = size;
    if (size > 0) {
...
static inline void lui_bytes_free (LvtkBytes* b) {
    b->size = 0;
    if (b->data != NULL) {
        free (b->data);
        b->data = NULL;
...
static inline uint8_t lui_bytes_get (LvtkBytes* b, lua_Integer index) {
    return b->data[index];
}

static inline void lui_bytes_set (LvtkBytes* b, lua_Integer index, uint8_t value) {
    b->data[index] = value;
}
...
static int f_new (lua_State* L) {
    ...
    luaL_setmetatable (L, LUI_MT_BYTE_ARRAY);
    size_t size = lua_isnumber (L, 1) ? (size_t) lua_tonumber (L, 1) : 0;
    lui_bytes_init (b, size);
    return 1;
}
...
static int f_get (lua_State* L) {
    ...
    lua_pushinteger (L, (lua_Integer) lui_bytes_get (b, index - 1));
    return 1;
}
...
static int f_set (lua_State* L) {
    ...
    luaL_argcheck (L, b != NULL, 1, "`bytes' expected");
    luaL_argcheck (L, index >= 1 && index <= b->size, 2, "index out of range");
    lui_bytes_set (b, index - 1, (uint8_t) value);
    return 1;
}
...
LUI_LUALIB
int luaopen_lui_bytes (lua_State* L) {
    if (luaL_newmetatable (L, LUI_MT_BYTE_ARRAY)) {
