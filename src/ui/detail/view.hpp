...
public:
    ...
    /** Reset everything */
    void reset() {
        for (uint32_t i = 0; i < LUI_MAX_BUTTONS; ++i)
            reset (i);
    }
...
private:
    ...
    Modifier mods;
    ButtonState down[LUI_MAX_BUTTONS];
    ButtonState up[LUI_MAX_BUTTONS];
    int click_count[LUI_MAX_BUTTONS];
