...
/** A weak refrenceable object ptr.
    ...
    See the @see LUI_WEAK_REFABLE macro to easily implement boiler plate
    details.
...
public:
    WeakRef() = default;
    WeakRef (Obj* obj)
        : _status (obj != nullptr ? Obj::lui_weak_status (obj) : WeakStatus<Obj>()) {}
...
/**
    ...
    member name, use LUI_WEAK_REFABLE_WITH_MEMBER instead.
    ...
    private:
        LUI_WEAK_REFABLE (MyObject)
    };
    @endcode

    @see WeakRef, WeakStatus
*/
#define LUI_WEAK_REFABLE(klass) LUI_WEAK_REFABLE_WITH_MEMBER (klass, _weak_status)
